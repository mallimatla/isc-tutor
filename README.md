...
4. **`ANTHROPIC_MODEL` model rotation.** The runtime now defaults to `claude-sonnet-5` (the earlier `claude-sonnet-4-20250514` reached end-of-life and started returning 404s). The prompt-version log in Firestore captures which model authored which content, so future rotations stay fixable without losing history.
