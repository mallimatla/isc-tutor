...
- **`ANTHROPIC_MODEL` rotates as models age out.** The runtime now defaults to `claude-sonnet-5` (the earlier `claude-sonnet-4-20250514` reached end-of-life and started returning 404s). Every prompt-version + model is recorded in Firestore alongside the content it generated, which means a rotation is fixable without losing provenance.
